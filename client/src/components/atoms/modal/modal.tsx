...
const Modal = ({ button, content, size = "md" }: ModalProps) => {
  ...
  return (
    <>
      ...
      <CSSTransition nodeRef={contentRef} in={showModal} timwout={200} classNames="fade-in" unmountOnExit children={
          <div className="fixed top-0 left-0 right-0 bottom-0 flex items-center justify-center z-10">
            ...
            <div onClick={() => setShowModal(false)} className="absolute top-0 left-0 right-0 bottom-0 bg-black opacity-50 z-0"/>
          </div>
        }
      />
    </>
