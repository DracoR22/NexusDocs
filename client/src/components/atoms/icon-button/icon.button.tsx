...
const IconButton = ({ icon, tooltip, onClick }: IconButtonProps) => {
  ...
  return (
    <div onMouseOver={() => setShowToolTip(true)} onMouseLeave={() => setShowToolTip(false)} className="relative flex justify-center items-center">
      <button onClick={() => onClick()} className="text-gray-600 flex justify-center items-center w-7 h-7 rounded hover:bg-gray-100">
        {icon}
      </button>
      {showTooltip && (
