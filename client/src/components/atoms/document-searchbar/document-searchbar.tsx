...
const DocumentSearchBar = () => {
  ...
  return (
    <div className={`${isFocused ? "bg-white shadow-xl" : "bg-gray-100"} w-full max-w-2xl rounded-md h-12 flex items-center text-gray-500 mr-4`}>
      ...
      <input onFocus={() => setIsFocused(true)} onBlur={() => setIsFocused(false)} type="text"
      className={`${isFocused ? "bg-white outline-none" : "bg-gray-100"}  w-full h-full pr-4 font-medium`} placeholder="Search" name="" id=""/>
    </div>
  )
}
