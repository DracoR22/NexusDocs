...
const Sharedusers = ({ documentUsers, setDocument }: SharedUserProps) => {
  ...
  return (
    ...
    {documentUsers.map((documentUser) => {
      return (
        <div key={documentUser.user.email} className="px-2 py-4 w-full flex items-center justify-between hover:bg-gray-100 rounded-md">
          <div className="flex items-center space-x-2">
          ...
          <button onClick={() => removeDocumentUser({ documentId: documentUser.documentId, userId: documentUser.userId })}
           disabled={loading} className="font-semibold text-blue-600 p-2 hover:bg-blue-50 rounded-md">
            Remove
          </button>
        </div>
      );
