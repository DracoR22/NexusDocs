...
class UserValidator {
    constructor() {
        this.register = [
            ...
            (0, express_validator_1.body)('password1').isLength({ min: 8, max: 25 }).withMessage('Password must be between 8 to 25 characters'),
            (0, express_validator_1.body)('password1').matches(/\d/).withMessage('Password must contain at least 1 number'),
        ...
        this.confirmResetPassword = [
            (0, express_validator_1.body)('password1').isLength({ min: 8, max: 25 }).withMessage('Password must be between 8 to 25 characters'),
            (0, express_validator_1.body)("password1").matches(/\d/).withMessage("Password must contain atleast 1 number"),
            (0, express_validator_1.body)("password2").custom((value, { req }) => {
                if (value !== req.body.password1) {
                    throw new Error("Passwords must match.");
                }
