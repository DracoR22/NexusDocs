...
class ShareValidator {
    constructor() {
        this.create = [
            (0, express_validator_1.body)('email').isEmail().normalizeEmail().withMessage('Provide a valid email to share this document with.'),
            (0, express_validator_1.body)('permission').custom((value) => {
                if (!Object.values(permission_enum_1.default).includes(value))
                    throw new Error('Provide a valid document permission');
                else
                    return true;
            })
        ];
